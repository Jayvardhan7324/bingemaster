...
playIcon.classList.add('https://replit.com/@JayVardhan2/Bingflix-Website#assets/dhoni.jpg');

// Create a new stylesheet
const sheet = new CSSStyleSheet();
sheet.replaceSync(Object.entries(styles).map(([selector, rules]) => `${selector} { ${Object.entries(rules).map(([property, value]) => `${property}: ${value};`).join(' ')} }`).join(' '));
...
const korean_series = `https://api.themoviedb.org/3/discover/tv?api_key=${apiKey}&language=ko-KR&region=KR&sort_by=popularity.desc&with_original_language=ko`;
      ...
      fetch(url)
        ...
        .then(data => {
          ...
          data.results.forEach(movie => {
            ...
            image.style.borderRadius='1.1rem';
  ...
  function activate(e) {
    ...
    document.querySelector(`.slider .item:nth-child(${a})`).style.opacity = 1;
    ...
    document.querySelector(`.slider .item:nth-child(6)`).style.opacity = 1;


});

nav.addEventListener('mouseleave', () => {
    // Make only movies 2 to 6 invisiblebe
 ...
 function adjustImageHeights() {
  ...
  images.forEach(image => {
      if (document.body.clientWidth <= 768) {
          image.style.height = "35vh"; // Decrease the height for mobile devices
      } else {
          image.style.height = "50vh"; // Default height for larger screens
      }
  });
}
